...
rl.on('line', function (input) {
    clear()
//Input -> [action, object, other object]
...
// var room = []
var shelf = new RoomObject("shelf", "it's dusty", ["key", "pen"], [], "search");
var drawer = new RoomObject("drawer", "it's locked", ["flashlight", "rope"], ["key"], "search");
var closet = new RoomObject("closet", "it's dark and damp", ["necklace"], ["flashlight"], "search");
var necklace = new RoomObject("necklace", "there's something inside", ["passcode"], [], "search");
var door = new RoomObject("Door", "it's locked, might need a passcode or something", [], ["passcode"], "search");

/* Create an array of Room objects < - why?
"A Room is a level in the game. A RoomObject is something that is contained inside that level - 
shelf, drawer, basement, attic, etc. Items  are needed to be able  to interact with some RoomObjects.  
Item can be found when interacting with a RoomObject. A Game consists of a series of Rooms. 
For this assignment you only need to have a single level. To enable multiple rooms you would need to make some changes in the code.*/
var bedroom = new Room([shelf, drawer, closet, necklace, door]);
...
/* Complete the code starting on line 323 - follow the comments. 

 Create a github repo with the complete code and share the link to that repo. 
 
 Bonus: 

Log all available actions / log available actions for each object */